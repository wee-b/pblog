          ...
          <template #default="{ row }">
            ...
            <el-dropdown @command="(cmd) => handleCommand(cmd, row)">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <!-- 0: 草稿 -> 投稿 -> 2 -->
                  <el-dropdown-item v-if="row.status === '0'" command="submit">
                    <el-icon><Upload /></el-icon>投稿
                  </el-dropdown-item>

                  <!-- 2: 待审核 -> 撤回 -> 0 -->
                  <el-dropdown-item v-if="row.status === '2'" command="cancel">
                    <el-icon><RefreshLeft /></el-icon>撤回投稿
                  </el-dropdown-item>

                  <!-- 2: 待审核 -> 审核通过 -> 1 -->
                  <el-dropdown-item v-if="row.status === '2'" command="pass" style="color: #67c23a;">
                    <el-icon><Check /></el-icon>审核通过
                  </el-dropdown-item>

                  <!-- 1: 已发布 -> 下架 -> 0 -->
                  <el-dropdown-item v-if="row.status === '1'" command="offline" style="color: #e6a23c;">
                    <el-icon><Download /></el-icon>下架文章
                  </el-dropdown-item>
...
import {
  ...
  Picture, ArrowDown, Upload, RefreshLeft, Download, Check // 引入新增的图标
...
// enableDisableArticle逻辑：待审核-->草稿  （取消投稿）；已发布-->草稿  （下架）；草稿-->待审核  （投稿）
// statusPass逻辑：待审核-->已发布
// --- 常量定义 ---
...
// 1. 保持您原始的 getList 逻辑完全不变
const getList = async () => {
  loading.value = true
  try {
    if (!userInfo.value || userInfo.value.username === null) {
      ElMessage.error("请先登录")
    }
    queryParams.username = userInfo.value.username
...
const handleCommand = (command, row) => {
  switch (command) {
    ...
    case 'cancel':  // 待审核 -> 撤回
    case 'offline': // 已发布 -> 下架
      // 这三种情况走 enableDisableArticle
      handleStatusChange(row, command)
      break
    case 'pass':    // 待审核 -> 通过
      // 走 statusPass
      handlePassArticle(row)
      break
    case 'delete':
      handleDelete(row)
      break
 ...
 * 调用 statusPass 接口
 */
const handlePassArticle = async (row) => {
  try {
    // 增加一个简单的确认框
    await ElMessageBox.confirm(`确认让文章《${row.title}》审核通过并发布吗？`, '审核确认', {
      confirmButtonText: '通过',
      cancelButtonText: '取消',
      type: 'success'
    })

    const res = await statusPass(row.id)
    if (res.data?.code === 200 || res.code === 200) {
