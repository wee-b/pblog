package com.pblog.common.constant;


public class DefaultConstants {

    // 0代表启用，1代表禁用
    public static final String DEFAULT_STATUS = "0";
    public static final String Banned_Status = "1";
    // 文章0待审核 1已发布 2草稿
    public static final String toInspect = "0";
    public static final String Already_handout = "1";
    public static final String Draft_Status = "2";
    public static final String Comment_Rejected = "2";
    // 逻辑删除标识: 1代表已删除，0代表未删除
    public static final String DEFAULT_DELFLAG = "0";
    public static final String Deleted_DelFlag = "1";

    public static final Integer ZERO = 0;

    public static final String unSticky = "0";
    public static final String isSticky = "1";
    public static final String unFeatured = "0";
    public static final String isFeatured = "1";


    public static final String Default_password = "123";

}
