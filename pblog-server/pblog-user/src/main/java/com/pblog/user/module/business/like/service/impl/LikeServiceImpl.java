package com.pblog.user.module.business.like.service.impl;
...
//            log.info("生产者发送了一条消息{}", sendResult);
    ...
    public Integer getLikeCount(String targetType, Integer targetId) {
        ...
        if (lc == null) {
            LikeCount lcEntity = new LikeCount();
            lcEntity.setTargetId(targetId);
            lcEntity.setTargetType(targetType);
            lcEntity.setTotal(0);
            likeCountMapper.insert(lcEntity);
        }

        String count = (lc == null) ? "0" : lc.getTotal().toString();
