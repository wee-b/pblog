...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public PageResult pageQuery(PageQueryDTO pageQueryDTO) {
    	...
    	IPage<Comment> pageRes = commentMapper.selectPage(page, null);

        List<CommentVO> collect = pageRes.getRecords().stream().map(
                one -> {
        ...
        return new PageResult(
                pageRes.getTotal(),    // 总记录数
                pageRes.getPages(),    // 总页数
                pageRes.getCurrent(),  // 当前页码
                pageRes.getSize(),     // 每页条数
                collect              // 当前页数据
        );
    }
